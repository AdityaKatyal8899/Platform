...
- The SDK only **emits** outcome signals (status / delivery URL / metadata / renditions / error)
...
### `SDKConfig` (dataclass)
**File:** `cowatch_sdk/config.py` (line 17)
**Task:** Describes a single HLS stream (one rung of the ladder).
**Fields:**
- `name: str` — e.g. `"240p"`, `"source"`.
- `width: int`, `height: int` — target frame size. `0,0` is allowed for the native/source rung.
- `video_bitrate: str` — e.g. `"600k"`.
- `audio_bitrate: str = "128k"`.
- `bufsize: Optional[str] = None` — explicit encoder buffer; if `None` it is derived (2× bitrate).
- `native: bool = False` — `True` ⇒ passthrough source resolution, no scale filter.

**Method `bufsize_or_default() -> str`**: returns the explicit `bufsize` or a default of `2 × video_bitrate`.

### `_parse_bitrate(s: str) -> int`
**File:** `cowatch_sdk/config.py` (line 32)
**Task:** Convert a bitrate string (`"600k"`, `"1m"`, `"500000"`) into an integer (bits/s). Internal helper.

### `_scale_bitrate(s: str, mult: float) -> str`
**File:** `cowatch_sdk/config.py` (line 41)
**Task:** Multiply a bitrate string by `mult` and return a `"Nk"` string. Used for the default buffer size. Internal helper.

### `_bitrate_for_height(h: int) -> str`
**File:** `cowatch_sdk/config.py` (line 45)
**Task:** Pick a sensible default video bitrate for a native source-resolution passthrough rung,
based on source height (`≤480→1000k`, `≤720→2500k`, `≤1080→5000k`, `≤1440→8000k`, else `12000k`). Internal helper.

### `HLS_SETTINGS` (module constant)
**File:** `cowatch_sdk/config.py` (line 58)
**Task:** The mobile-first ladder used when none is configured:
`240p@426×240/600k`, `360p@640×360/1000k`, `720p@1280×720/2500k`.

### `adapt_ladder(ladder: list, src: dict) -> list`
**File:** `cowatch_sdk/config.py` (line 65) — **module function, not a method**
**Task:** Adapt the ladder to the *actual* source resolution (source-aware HLS):
- **Never upscale:** drop any rung whose height exceeds the source height.
- If the source is **taller than the top rung**, append a true **source-resolution native rung** so high-res uploads keep real quality.
- If the source is **smaller than every rung**, fall back to a single native source rung.
- If probing failed (`height == 0`), return the ladder unchanged (safe fallback).
**Params:** `ladder` = list of `SDKConfig`; `src` = probe dict `{width, height, duration, has_audio}`.
**Returns:** the adapted list of `SDKConfig`.

### `_parse_abr_ladder(spec: Optional[str]) -> Optional[list]`
**File:** `cowatch_sdk/config.py` (line 89)
**Task:** Parse `"name:width:height:vbitrate[:abitrate]"` comma-separated profiles (e.g. from `HLS_LADDER` env) into a list of `SDKConfig`. Returns `None` when `spec` is empty. Internal helper.
...
- *HLS:* `hls_playlist_type=True`, `abr_ladder=None` (uses `HLS_SETTINGS` when `None`).
- *Runtime:* `temp_dir="storage/videos"`, `upload_window=10`, `preplay_threshold=0.30`, `retry_attempts=5`, `retry_backoff=1.0`.

**Method `effective_ladder() -> list`**: resolves the rungs actually used.
- If `hls_playlist_type` is `False` → returns a single native source rung (legacy single-stream behavior, still master-wrapped).
- Else returns `abr_ladder` or `HLS_SETTINGS`.
- Note: source-aware adaptation (`adapt_ladder`) is applied later in the pipeline, not here.
...
**Task:** Inverse of `to_dict()` — rebuild an `SDKConfig` from a dict, reconstructing `SDKConfig` objects from the `abr_ladder` list. Used on the worker side of a queue.
...
#### `_build_ffmpeg_args(config, input_path, output_dir, ladder, has_audio) -> list`
**File:** `cowatch_sdk/pipeline/transcode.py` (line 21)
**Task:** Build the ffmpeg argument list for the resolved HLS ladder. Always emits a
`master.m3u8` + one `v{i}.m3u8` variant per rendition + per-rendition segments
(`v{i}_seg_%03d.ts`), with keyframes aligned across renditions for clean HLS streaming.
- **Multi-rendition branch:** uses `filter_complex` with aspect-aware `scale+pad` for non-native
  rungs; native rungs are mapped without scaling. Critical detail: audio is mapped **once per
  variant** using the per-type index `a:0` in `-var_stream_map` (e.g. `"v:0,a:0 v:1,a:0"`).
- **Single native branch** (`hls_playlist_type=False`): one native rung, `var_stream_map "v:0,a:0"`.
...
4. Cleans old artifacts, resolves `effective_ladder()`, applies `adapt_ladder()` (if HLS on),
   builds ffmpeg args.
5. Starts the parallel `run_uploader` thread, runs ffmpeg (watching for uploader errors).
6. On completion uploads all `.m3u8` (master + variants), emits `delivery_url`
   (`master.m3u8` URL), emits `renditions` (per-variant URLs), sets `status=ready`.
...
source-aware ladder adaptation. On any failure returns safe defaults
...
> - `{event:"renditions", renditions}` — list of `{name, width, height, video_bitrate, audio_bitrate, url}`
> - `{event:"error", error}`
...
- `on_renditions(video_id, renditions)` — **optional** (default no-op); override to deliver it.
...
# events now holds status / metadata / delivery_url / renditions
```
...
| `SDKConfig` | `config.py` | One HLS stream description |
| `HLS_SETTINGS` | `config.py` | Mobile-first default ladder |
| `adapt_ladder` | `config.py` | Source-aware ladder (no upscale, native passthrough) |
| `_bitrate_for_height` | `config.py` | Default bitrate for a source rung |
| `_parse_abr_ladder` | `config.py` | Parse `HLS_LADDER` env string |
| `effective_ladder` | `config.py` | Resolve rungs (HLS on/off) |
...
| `_build_ffmpeg_args` | `pipeline/transcode.py` | Build ffmpeg HLS command |
