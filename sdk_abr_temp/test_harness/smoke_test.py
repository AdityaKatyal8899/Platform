...
Processes the video locally and prints the result-signal events.
...
def main():
    ...
    print(f"Processing {src} as {video_id} (local basic HLS)...")
    try:
    ...
    for f in files:
        print("  ", f)
    assert "stream.m3u8" in files, "stream.m3u8 missing!"
    print(f"\nSMOKE TEST PASSED — stream playlist successfully generated.")
    print("Open the harness UI to play it, or point an HLS player at:")
    print(f"  {out_dir}/stream.m3u8")
