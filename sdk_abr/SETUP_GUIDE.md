...
| `KEYFRAME_INTERVAL` | `2.0` | Seconds between forced keyframes (HLS segment alignment) |
...
```
- Select a video → **Submit** → watch pipeline logs → the player appears and begins playback.
...
```
Prints produced files → confirms pipeline works end-to-end.
...
└─────────────────────┘     └──────────────────────┘     └─────────────────────┘
        ...
        │  2. Creates video_id       │  3. ffmpeg HLS (single rendition)        │
...
**Key boundaries:**
- **SDK owns**: transcode logic, HLS packaging, segment upload, keyframe alignment, result signaling.
...
sdk/
├── cowatch_sdk/
│   ├── __init__.py           # exports: SDKConfig, process_video_to_hls, CallbackResultSink, ...
│   ├── config.py             # SDKConfig, load_config, build_adapters, config_from_dict, ...
│   ├── pipeline/
│   │   ├── transcode.py      # run_pipeline, _build_ffmpeg_args (single-rendition)
...
| `seg_%03d.ts` 404 in player | Old single-rendition URL used | SDK emits `master.m3u8` + `v0.m3u8` — player must load `master.m3u8`. |
...
5. **Persist result events** (`delivery_url`, `metadata`) into your DB.
6. **Serve HLS** via your CDN (or same-origin `/hls/...` route for local).
7. **Playback** — any HLS player (hls.js, video.js, native Safari) works by loading `master.m3u8`.
