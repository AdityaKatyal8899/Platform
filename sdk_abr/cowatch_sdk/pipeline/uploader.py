...
def run_uploader(output_dir, video_id, storage: StorageBackend, result: ResultSink,
    ...
    seg_pattern = re.compile(r"^v(\d+)_seg_(\d+)\.ts$")
    ...
    try:
        while not stop_event.is_set() or any(
            ...
            try:
                ...
                # Group files by rendition index to track the max segment sequence number for each
                rendition_max_seq = {}
                parsed_segments = []  # list of (rendition, seq, filename)
                
                for f in all_files:
                    m = seg_pattern.match(f)
                    if m:
                        rendition = int(m.group(1))
                        seq = int(m.group(2))
                        parsed_segments.append((rendition, seq, f))
                        if rendition not in rendition_max_seq or seq > rendition_max_seq[rendition]:
                            rendition_max_seq[rendition] = seq
                ...
                uploadable = []
                for rendition, seq, fname in parsed_segments:
                    if fname in uploaded_files:
                        continue
                    # Upload only if seq is strictly less than the max sequence number written
                    # for that rendition (guaranteeing that FFmpeg has fully finished writing the segment),
                    # or if the transcode process has finished.
                    if seq < rendition_max_seq.get(rendition, 0) or stop_event.is_set():
                ...
                if len(uploadable) >= window_size or (stop_event.is_set() and len(uploadable) > 0):
                    ...
                    # Upload all HLS playlists (variant .m3u8 + master.m3u8) so they
                    # stay current as ffmpeg appends segments.
